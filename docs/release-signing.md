...
На затронутых прошивках ColorOS/OxygenOS версии `1.2.0` и `1.2.1` не передают системному
staging-компоненту доступ к скачанному файлу. Поэтому первую версию с исправленным URI-grant
(`1.2.2`) на таких устройствах также нужно установить вручную из GitHub Release. Последующие
...
3. Для проверки перехода на текущий `v1.2.1` укажите `test_version_name = 1.2.0` и
   `test_version_code = 3`. Оба значения должны быть ниже опубликованных `1.2.1` и `4`.
4. Скачайте созданный Actions artifact `ValerochkaGym-release-…`, распакуйте его и установите
   `ValerochkaGym-v1.2.0.apk` на чистый эмулятор или отдельное тестовое устройство.
5. Запустите приложение и проверьте предложение обновления:
   ...
   - **«Обновить»** — пройдите выдачу разрешения источнику, дождитесь системного установщика и
     подтвердите установку. В настройках должна появиться версия `1.2.1`.
6. Для повторного прогона удалите установленную `1.2.1` и снова установите тестовую `1.2.0`:
   Android не разрешает обычное понижение `versionCode` поверх новой версии.

То же самое через ADB:

```bash
adb install -r /path/to/ValerochkaGym-v1.2.0.apk
...
значениями, равными текущему опубликованному релизу (`1.2.1` и `4`), установите artifact на чистое
