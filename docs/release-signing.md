...
3. Для проверки перехода на текущий `v1.2.0` укажите `test_version_name = 1.1.9` и
   `test_version_code = 2`. Оба значения должны быть ниже опубликованных `1.2.0` и `3`.
4. Скачайте созданный Actions artifact `ValerochkaGym-release-…`, распакуйте его и установите
   `ValerochkaGym-v1.1.9.apk` на чистый эмулятор или отдельное тестовое устройство.
5. Запустите приложение и проверьте предложение обновления:
   ...
   - **«Обновить»** — пройдите выдачу разрешения источнику, дождитесь системного установщика и
     подтвердите установку. В настройках должна появиться версия `1.2.0`.
6. Для повторного прогона удалите установленную `1.2.0` и снова установите тестовую `1.1.9`:
   Android не разрешает обычное понижение `versionCode` поверх новой версии.

То же самое через ADB:

```bash
adb install /path/to/ValerochkaGym-v1.1.9.apk
...
значениями, равными текущему опубликованному релизу (`1.2.0` и `3`), установите artifact на чистое
