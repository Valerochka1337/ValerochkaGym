 ...
 * (см. [SheetsApi]/[CalendarApi]), поэтому OkHttp-клиент без авторизующего интерсептора. У Sheets,
 * Calendar и пользовательского OpenAI-совместимого API разные адреса, поэтому используются
 * отдельные Retrofit-инстансы. Долгие запросы к модели получают свой OkHttp-клиент, чтобы не
 * растягивать таймауты обычных Google-запросов.
 */
@Module
...
object NetworkModule {
  ...
  /**
   * Генерация может долго не присылать ни одного байта, а фото дополнительно требует времени на
   * отправку. Общий дедлайн остаётся конечным; отмена coroutine немедленно отменяет Call.
   */
  @Provides
  @Singleton
  @Named("ai")
  fun provideAiOkHttpClient(client: OkHttpClient): OkHttpClient =
      client
          .newBuilder()
  ...
  @Provides
  @Singleton
  fun provideAiApi(@Named("ai") retrofit: Retrofit): AiApi = retrofit.create(AiApi::class.java)
}
