...
import com.valerochka1337.valerochkagym.domain.ParsedRoutineRows
import com.valerochka1337.valerochkagym.domain.ParsedRows
 ...
 *
 * [Success] — импортировано [Success.imported] новых тренировок и, при наличии, замеры/программы.
 * [NothingToImport] — импортировать нечего (нет листов/строк, или данные уже актуальны). [Failure]
 * — ошибка (reason показывается пользователю).
 */
 ...
 */
class WorkoutImportRepositoryImpl
@Inject
constructor(
  ...
  override suspend fun importAll(): ImportResult {
    val spreadsheetId =
        settingsRepository.settings.first().spreadsheetId
            ?: return ImportResult.Failure("Укажите таблицу в настройках")

    val token =
        when (val result = googleAuth.getAccessToken()) {
          is TokenResult.Success -> result.token
          TokenResult.NeedsConsent ->
              return ImportResult.Failure("Настройте доступ к Google в настройках")
    ...
    return try {
      val sheetTitles =
          api.getSpreadsheet(bearer, spreadsheetId).sheets.map { it.properties.title }.toSet()
      val workouts =
          if (WORKOUTS_SHEET in sheetTitles) {
            WorkoutRowParser.parse(
      ...
      val measurements =
          if (MEASUREMENTS_SHEET in sheetTitles) {
      ...
      val gyms =
          if (GymSheetRowMapper.SHEET_NAME in sheetTitles) {
      ...
      val routineGyms =
          if (RoutineGymsSheetRowMapper.SHEET_NAME in sheetTitles) {
            RoutineGymsSheetRowParser.parse(
                api.getValues(bearer, spreadsheetId, RoutineGymsSheetRowMapper.RANGE)
                    .values
                    .orEmpty(),
            )
          } else {
      ...
      var skippedRows =
          workouts.skippedRows +
              ...
              routines.skippedRows +
              exercises.skippedRows +
              gyms.skippedRows +
              routineGyms.skippedRows
      if (
      ...
      val existingMeasurements =
          database.bodyMeasurementDao().observeAll().first().mapTo(mutableSetOf()) { it.id }
      val freshMeasurements = measurements.measurements.filterNot { it.id in existingMeasurements }
      val existingRoutines =
          database.routineDao().observeRoutinesFull().first().associateBy { it.routine.syncId }
      ...
      database.withTransaction {
        exercises.records.forEach { record -> if (applyExercise(record)) importedExercises++ }
        byName =
            exerciseDao.getAllOnce().associateTo(mutableMapOf()) { it.name.lowercase() to it.id }
      ...
      if (
          importedWorkouts +
              ...
              importedRoutines +
              importedExercises +
              importedGyms == 0 && skippedRows == 0 && importedRoutineGyms == 0
      ) {
        ImportResult.NothingToImport
      } else {
  ...
  private suspend fun applyExercise(record: ExerciseSheetRecord): Boolean {
    ...
    return when (record) {
      ...
      is ExerciseSheetRecord.Snapshot -> {
        ...
        if (
            existing?.let(CanonicalExerciseRegistry::isBuiltIn) == true ||
                CanonicalExerciseRegistry.entries.any { it.exercise.syncId == record.syncId }
        )
            return false
        ...
        val exerciseId =
            ...
            else existing.id.also { exerciseDao.update(entity) }
  ...
  private suspend fun applyRoutineGyms(record: RoutineGymsSheetRecord): ApplyConfigurationResult {
    val routine =
        database.routineDao().observeRoutinesFull().first().firstOrNull {
          it.routine.syncId == record.routineSyncId
        }
    ...
    val snapshot =
        record as? RoutineGymsSheetRecord.Snapshot ?: return ApplyConfigurationResult.Ignored
  ...
  /**
   * Последний барьер транзакции импорта. Промежуточный порядок применения удалённых снимков не
   * важен: наружу попадёт только целиком совместимый aggregate, иначе Room откатит всё.
   */
  private suspend fun validateConfigurationAggregate() {
    ...
    workoutDao.getActiveWorkoutId()?.let { workoutId ->
      ...
      if (unavailable.isNotEmpty()) {
        conflicts +=
            "Активная тренировка «${workout.workout.name}»: " + unavailable.joinToString { it.name }
      }
    }
  ...
  private suspend fun insertWorkout(
    ...
    for (exercise in parsed.exercises) {
      ...
      val exerciseId =
          exercise.exerciseSyncId?.let { syncId -> bySyncId[syncId] }
              ?: byName[key]
              ?: createExercise(
                      name = exercise.name,
      ...
      val workoutExerciseId =
          workoutDao.insertWorkoutExercise(
              WorkoutExerciseEntity(
                  ...
                  sectionId = exercise.sectionId ?: UUID.randomUUID().toString(),
  ...
  /**
   * Создаёт отсутствующее упражнение вместе с картой мышц. В Sheets есть только группа, поэтому для
   * знакомого имени карта точная, а для нового берётся типичная для группы (`muscleRows`).
   */
  private suspend fun createExercise(
    ...
    val entity =
        ExerciseEntity(
            ...
            // Временный fallback из legacy Routines всегда уступает полноценному Exercises
            // snapshot.
            updatedAt = 1,
        )
