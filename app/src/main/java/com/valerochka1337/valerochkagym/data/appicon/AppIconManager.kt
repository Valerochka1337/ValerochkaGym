 ...
 *
 * Android не умеет перекрашивать иконку на лету: в манифесте объявлены четыре `activity-alias`
 * (по одному на [AccentColor]), и «сменой иконки» называется включение нужного и выключение
 * остальных. Источник правды — настройка в DataStore, поэтому менеджер не вызывается из UI, а
 * подписан на неё ([startSync]): так иконка чинится и после переустановки, где состояние
 * компонентов сбрасывается к манифестному.
 *
 * Переключение отложено до ухода приложения в фон. Выключение alias'а, из которого запущена
 * текущая задача, система понимает как «компонента больше нет» и сносит задачу целиком: выбрал
 * цвет — вылетел на рабочий стол. В фоне та же операция стоит лишь пропажи карточки из «недавних».
 */
@Singleton
class AppIconManager @Inject constructor(
    ...
    /**
     * Включает alias выбранного акцента и гасит остальные. Сначала включение, потом выключение:
     * в промежутке, когда не включён ни один launcher-компонент, лаунчер убирает приложение из
     * списка и может не вернуть его обратно.
     */
