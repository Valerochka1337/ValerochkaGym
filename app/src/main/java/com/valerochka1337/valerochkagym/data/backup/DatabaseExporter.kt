...
/**
 * Экспорт локальной базы в выбранный пользователем документ. Интерфейс — шов для тестов ViewModel
 * (как [com.valerochka1337.valerochkagym.worker.UploadScheduler]).
 */
 ...
 *
 * Перед копированием WAL сбрасывается в основной файл (`wal_checkpoint(TRUNCATE)`) — иначе копия
 * `gym.db` без `-wal` потеряла бы последние записи. Копия — обычный SQLite-файл, который
...
constructor(
  ...
  override suspend fun export(target: Uri): ExportResult =
      withContext(Dispatchers.IO) {
        try {
          database.query(SimpleSQLiteQuery("PRAGMA wal_checkpoint(TRUNCATE)")).use {
            it.moveToFirst()
          }
