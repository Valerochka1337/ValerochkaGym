...
class ConfigurationSheetsRepositoryImpl
@Inject
constructor(
  ...
  override suspend fun uploadExercise(syncId: String): UploadResult {
    val exercise =
        exerciseDao.getAllOnce().firstOrNull { it.syncId == syncId }
  ...
  override suspend fun uploadGym(syncId: String): UploadResult {
    val gym = gymDao.getGymBySyncId(syncId) ?: return UploadResult.PermanentFailure("Зал не найден")
    val full =
        gymDao.getGymWithExercises(gym.id) ?: return UploadResult.PermanentFailure("Зал не найден")
    return uploadVersion(
  ...
  private suspend fun uploadVersion(
    ...
    return try {
      ...
      val existingVersion =
          existing.filter { row -> row.matchesVersion(syncId, updatedAt, isDeleted) }
  ...
  private fun List<String>.matchesVersion(
      syncId: String,
      updatedAt: Long,
      isDeleted: Boolean,
  ): Boolean =
