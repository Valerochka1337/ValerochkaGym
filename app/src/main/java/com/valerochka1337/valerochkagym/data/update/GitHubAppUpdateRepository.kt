...
constructor(
  ...
  override suspend fun findUpdate(installedVersionName: String): AppRelease? =
      try {
  ...
  override suspend fun downloadAndVerify(
      ...
      withContext(Dispatchers.IO) {
        ...
        if (
            target.isFile &&
                target.length() == release.apk.sizeBytes &&
        ...
        target.delete()
        temporary.delete()
        directory.listFiles()?.filter { it != temporary && it != target }?.forEach(File::delete)
