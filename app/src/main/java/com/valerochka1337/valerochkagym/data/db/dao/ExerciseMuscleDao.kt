 ...
 * ([observeAll]) и считает эффективные подходы в памяти — карта маленькая (десятки упражнений
 * × ≤7 мышц), зато формулы (e1RM, скользящие окна, зоны объёма) остаются в чистом Kotlin.
 */
@Dao
interface ExerciseMuscleDao {

    @Query("SELECT * FROM exercise_muscles")
    fun observeAll(): Flow<List<ExerciseMuscleEntity>>
    ...
    @Upsert
    suspend fun upsertAll(rows: List<ExerciseMuscleEntity>)
