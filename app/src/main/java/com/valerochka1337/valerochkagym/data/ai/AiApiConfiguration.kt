...
@Singleton
class StoredAiApiConfigurationProvider
@Inject
constructor(
...
/**
 * Храним HTTP(S) base URL с завершающим `/`. Пользователь может вставить домен, base URL или один
 * из полных endpoint-ов. Если путь ещё не заканчивается на `/v1`, добавляем этот API-префикс; если
 * схема не указана, выбираем HTTPS.
 */
...
internal fun aiModelsEndpoint(baseUrl: String): String =
    requireNotNull(baseUrl.toHttpUrlOrNull()?.resolve("models")) { "Некорректный base URL" }
        .toString()
