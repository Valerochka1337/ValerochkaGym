 ...
 * операция закрывает три случая: чистая установка, апгрейд с v2 (миграция создаёт таблицу пустой) и
 * упражнения, приехавшие импортом из Sheets. Уже размеченные упражнения не трогаются — ручные
 * правки пользователя не перезатираются.
 */
suspend fun seedMissingExerciseMuscles(
  ...
  val rows =
      exerciseDao
          ...
          .flatMap { exercise ->
            CanonicalExerciseRegistry.loadsFor(exercise).orEmpty().map { load ->
...
/**
 * Registry-only reconciliation. Existing built-ins are found through the explicit identity bridge,
 * so their local ID, historic workout links, sync ID and every gym relation survive. Custom rows
 * are intentionally invisible to this operation.
 */
suspend fun reconcileCanonicalExerciseCatalog(database: GymDatabase) =
