...
interface GymDao {
  ...
  @Insert suspend fun insertGym(gym: GymEntity): Long

  @Update suspend fun updateGym(gym: GymEntity)

  /** Returns the number of deleted rows; foreign keys may reject a linked routine. */
  @Query("DELETE FROM gyms WHERE id = :id") suspend fun deleteGym(id: Long): Int
  ...
  @Insert suspend fun insertGymExercises(links: List<GymExerciseEntity>)
  ...
  @Insert suspend fun insertRoutineGyms(links: List<RoutineGymEntity>)
  ...
  @Insert suspend fun insertWorkoutGyms(links: List<WorkoutGymEntity>)
