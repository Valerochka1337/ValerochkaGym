 ...
 *
 * [Success] — запись уже в таблице или только что добавлена. [PermanentFailure] — повтор не
 * поможет; у тренировок и замеров локальная запись уже стала FAILED, у программ следующая правка
 * или ручная выгрузка создаст новую попытку. [TransientFailure] — сеть/429/5xx; окончательное
 * решение о ретрае принимает воркер.
 */
 ...
 *
 * Тренировки занимают лист `Workouts`, замеры — `Measurements`, программы — `Routines`. Каждая
 * запись имеет стабильный UUID, а у программ ещё и монотонную версию: это делает повтор WorkManager
 * безопасным без обновления строк. В частности, локальные правки и удаление замера не переписывают
 * историческую строку `Measurements`, а удаление программы добавляет tombstone в `Routines`.
 */
...
constructor(
    ...
    val token =
        when (val result = googleAuth.getAccessToken()) {
          ...
          TokenResult.NeedsConsent ->
              return permanentWorkout(workoutId, "Настройте доступ к Google в настройках")
          is TokenResult.Failed ->
              return UploadResult.TransientFailure(GoogleErrorMessages.NO_CONNECTION)
        }
  ...
  override suspend fun uploadMeasurement(measurementId: String): UploadResult {
    ...
    val measurement =
        bodyMeasurementDao.getById(measurementId)
    ...
    return try {
      ...
      val rows =
          if (measurementIdColumn.isEmpty()) {
  ...
  override suspend fun uploadRoutine(routineSyncId: String): UploadResult {
    val routine =
        routineDao.observeRoutinesFull().first().firstOrNull { it.routine.syncId == routineSyncId }
    ...
    return try {
      ...
      if (existing.isNotEmpty()) {
        when (existing.first()) {
          ...
          RoutineRowMapper.LEGACY_HEADER_ROW ->
              api.updateValues(
                  bearer = bearer,
  ...
  private suspend fun ensureMeasurementsSheet(
    ...
    spreadsheet.sheets
        .firstOrNull { it.properties.title == MEASUREMENTS_SHEET }
        ?.let {
    ...
    val workouts =
        spreadsheet.sheets.withIndex().firstOrNull { it.value.properties.title == WORKOUTS_SHEET }
    val indexAfterWorkouts =
        workouts?.let { indexed -> (indexed.value.properties.index ?: indexed.index) + 1 }
    try {
      api.batchUpdate(
          bearer,
          spreadsheetId,
              ...
              requests =
                  listOf(
                      BatchRequestDto(
                          addSheet =
                              AddSheetDto(
                                  SheetPropertiesDto(MEASUREMENTS_SHEET, indexAfterWorkouts)
                              ),
                      ),
                  ),
          ),
      ...
      if (e.code() == ADD_SHEET_CONFLICT && measurementsSheetExists(bearer, spreadsheetId)) {
        return api.getSpreadsheet(bearer, spreadsheetId)
            .sheets
  ...
  /**
   * v1 of `Measurements` had A:N. Add the v5 report columns only to a known app-managed legacy
   * header, inserting columns first so any user content to the right is shifted intact. Historical
   * data rows intentionally keep empty cells in the new fields.
   */
  private suspend fun ensureMeasurementsHeader(
    ...
    val header =
        api.getValues(bearer, spreadsheetId, MEASUREMENT_HEADER_RANGE)
            .values
            ?.firstOrNull()
            .orEmpty()
    ...
    val sheetId =
        sheet.sheetId ?: return "Не удалось определить лист Measurements для обновления заголовка"
    api.batchUpdate(
        bearer,
        spreadsheetId,
        BatchUpdateRequestDto(
    ...
    api.updateValues(
        ...
        body =
            UpdateValuesDto(
                jsonRows(
                    listOf(
                        BodyMeasurementRowMapper.HEADER_ROW.drop(LEGACY_MEASUREMENT_COLUMN_COUNT)
                    )
                )
            ),
    )
  ...
  private suspend fun measurementsSheetExists(bearer: String, spreadsheetId: String): Boolean =
      api.getSpreadsheet(bearer, spreadsheetId).sheets.any {
        it.properties.title == MEASUREMENTS_SHEET
      }
  ...
  private fun jsonRows(rows: List<List<Any?>>): JsonArray = buildJsonArray {
    rows.forEach { row ->
      add(
          buildJsonArray { row.forEach { cell -> add(cellToJson(cell)) } },
      )
    }
  }
  ...
  private fun cellToJson(cell: Any?): JsonPrimitive =
      when (cell) {
        null -> EMPTY_CELL
  ...
  private fun List<String>.hasStableRoutineExerciseReference(): Boolean {
    ...
    return runCatching { java.util.UUID.fromString(raw) }
        .getOrNull()
        ?.toString()
