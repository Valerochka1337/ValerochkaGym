...
class WeeklyScheduleRepositoryImpl @Inject constructor(
    ...
    override suspend fun save(schedule: WeeklySchedule): ScheduleResult = interactiveBoundary {
        ...
        finishPendingInteractive()?.let { return@interactiveBoundary it }

        val namedRules = mutableListOf<Pair<DayRule, String>>()
        for (rule in schedule.rules) {
            val name = routineDao.getRoutineName(rule.routineId)
        ...
        accountGate(account.active, account.currentEmail)?.let { return@interactiveBoundary it }
        val owner = normalizeEmail(account.active.ownerEmail).ifEmpty { account.currentEmail }
        val prepared = namedRules.map { (rule, name) ->
            val eventId = newEventId()
            val preparedRule = rule.copy(calendarEventId = eventId)
            PreparedCalendarEvent(eventId, preparedRule, recurringRequest(preparedRule, name, eventId))
        }
        val target = WeeklySchedule(prepared.map { it.rule }, ownerEmail = owner)
        val operation = WeeklyScheduleOperation(
    ...
    override suspend fun resumePendingOperation(): WeeklyScheduleRecoveryResult = mutex.withLock {
        try {
    ...
    private suspend fun clearLocked(): ScheduleResult {
        finishPendingInteractive()?.let { return it }
        val account = adoptLegacyOwnerOrReadActive()
        if (account.active.rules.isEmpty()) return ScheduleResult.Success
        accountGate(account.active, account.currentEmail)?.let { return it }
        val owner = normalizeEmail(account.active.ownerEmail).ifEmpty { account.currentEmail }
        val operation = WeeklyScheduleOperation(
    ...
    private suspend fun execute(
        ...
        try {
            if (operation.phase == WeeklyScheduleOperationPhase.CLEANUP_NEW && operation.cleanupNewIds.isEmpty()) {
                journal.clear()
                return Execution.Completed
            }
            if (operation.phase == WeeklyScheduleOperationPhase.DELETE_OLD && operation.pendingDeleteIds.isEmpty()) {
                commitTargetAndClear(operation)
                return Execution.Completed
            }
            if (operation.phase == WeeklyScheduleOperationPhase.CREATE_NEW && operation.pendingCreateIds.isEmpty()) {
            ...
            val bearer = when (val token = googleAuth.getAccessTokenForAccount(expectedEmail)) {
                is TokenResult.Success -> "Bearer ${token.token}"
                TokenResult.NeedsConsent -> return Execution.Consent
                is TokenResult.Failed -> return Execution.Retry(GoogleErrorMessages.NO_CONNECTION).also {
            ...
            for (eventId in operation.pendingDeleteIds.toList()) {
                when (val deletion = deleteOne(bearer, eventId)) {
                    ...
                    is DeleteOutcome.Retry -> return Execution.Retry(deletion.message).also {
                        enqueueFromInteractive(origin)
                    }
                }
            }
    ...
    private suspend fun deleteOne(bearer: String, eventId: String): DeleteOutcome = try {
        val response = api.deleteEvent(bearer, eventId)
        when {
            response.isSuccessful || response.code() in DELETE_CONFIRMED_CODES -> DeleteOutcome.Confirmed
    ...
    private fun accountGate(active: WeeklySchedule, currentEmail: String): ScheduleResult? {
        ...
        if (active.rules.isNotEmpty() && owner != currentEmail) return ScheduleResult.Failure(ACCOUNT_MISMATCH)
        return null
    }

    private fun recurringRequest(rule: DayRule, routineName: String, eventId: String): CalendarEventDto {
        val zone = ZoneId.systemDefault()
        val startMillis = nextOccurrenceMillis(rule.isoDay, rule.hour, rule.minute, zone, LocalDate.now(zone))
        ...
        return CalendarEventDto(
            ...
            end = EventDateTimeDto(start.plusSeconds(EVENT_DURATION_SECONDS).format(DateTimeFormatter.ISO_OFFSET_DATE_TIME), zone.id),
    ...
    private fun newEventId(): String = ByteArray(16).also(secureRandom::nextBytes)
    ...
    private fun decodeSchedule(raw: String?): WeeklySchedule = raw
        ?.let { runCatching { json.decodeFromString<WeeklySchedule>(it) }.getOrNull() }
        ?: WeeklySchedule()

    private fun executionToRecoveryResult(execution: Execution): WeeklyScheduleRecoveryResult = when (execution) {
    ...
    private fun executionToScheduleResult(execution: Execution): ScheduleResult = when (execution) {
    ...
    private enum class ExecutionOrigin { INTERACTIVE, RECOVERY }
    ...
    private companion object {
        ...
        const val ACCOUNT_MISMATCH = "Выбран другой Google-аккаунт — переключитесь на владельца расписания"
        ...
        const val INSERT_FAILED_OLD_PRESERVED = "Новое расписание не создано — старое расписание сохранено"
        const val INSERT_FAILED_CLEANUP_DEFERRED = "Старое расписание сохранено; очистка новых событий будет продолжена позже"
        ...
        const val LOCAL_STATE_ERROR = "Не удалось сохранить состояние расписания — старое расписание сохранено"
    }
}
