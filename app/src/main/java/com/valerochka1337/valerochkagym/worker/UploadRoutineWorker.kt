...
/**
 * Выгружает текущий неизменяемый снимок программы либо её tombstone-строку. Отдельного статуса в
 * Room здесь нет: следующая правка или действие «Выгрузить всё» всегда безопасно ставят актуальную
 * версию повторно, а Sheets дедуплицирует пару UUID + updatedAt.
 */
@HiltWorker
class UploadRoutineWorker
@AssistedInject
constructor(
  ...
  companion object {
    ...
    private fun enqueue(
      ...
      val request =
          OneTimeWorkRequestBuilder<UploadRoutineWorker>()
              .setConstraints(
