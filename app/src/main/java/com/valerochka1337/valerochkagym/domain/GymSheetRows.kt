...
object GymSheetRowMapper {
    ...
    fun rows(record: GymSheetRecord): List<List<Any?>> = when (record) {
    ...
    private fun snapshotRows(snapshot: GymSheetRecord.Snapshot): List<List<Any?>> {
        ...
        val exerciseIds = snapshot.exerciseSyncIds
...
object GymSheetRowParser {
    private data class SnapshotBuilder(
        ...
        fun accept(row: List<String>): Boolean {
            ...
            if (deleted) {
                if (row.sheetCell(GYM_NAME).isNotEmpty() || row.sheetCell(EXERCISE_ID).isNotEmpty()) return reject()
