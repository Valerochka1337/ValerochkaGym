 ...
 * `updated_at`; равная версия остаётся последней встретившейся в листе. Это даёт повторной выгрузке
 * и догоняющему воркеру идемпотентное поведение.
 */
object RoutineRowParser {
  ...
  fun parse(rows: List<List<String>>): ParsedRoutineRows {
    ...
    rows.forEach { row ->
      ...
      val exerciseId =
          if (rawExerciseId.isEmpty()) {
            null
          } else {
            canonicalSheetUuidOrNull(rawExerciseId)
                ?: run {
                  skippedRows++
                  return@forEach
                }
    ...
    val routines =
        snapshots.values.mapNotNull { versions ->
