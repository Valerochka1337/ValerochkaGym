...
/**
 * Итог разбора листа: дерево тренировок плюс счётчик [skippedRows] — строк с `workout_id`, которые
 * не удалось разобрать (битое время). Счётчик всплывает до пользователя: молчаливый пропуск
 * превращал бы кривую таблицу в «нечего импортировать».
 */
 ...
 * нераспознанным временем (`date`+`start_time`) считаются в [ParsedRows.skippedRows]. Строки
 * группируются по `workout_id` (порядок появления сохраняется), внутри — по имени упражнения в
 * порядке первого появления (это даёт `position`). `startedAt`/`finishedAt` тренировки —
 * минимум/максимум `completedAt` её подходов. Числа парсятся мягко (запятая-десятичный разделитель
 * допускается); пустая ячейка → `null`.
 */
object WorkoutRowParser {
  ...
  fun parse(rows: List<List<String>>): ParsedRows {
    ...
    return ParsedRows(
        ...
        fatalError =
            if (malformedCurrent) "Строки Workouts A:S содержат некорректный снимок секции"
            else null,
    )
  }
