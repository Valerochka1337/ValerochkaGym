...
/**
 * Иконка упражнения по снаряду/движению. Ресурс выводится из названия (ключевые слова), поэтому не
 * требует поля в БД и работает и для пользовательских упражнений.
 *
 * Порядок правил важен: кардио-тренажёры и «гиря» проверяются раньше общих снарядов, а «блок» —
 * раньше «тренажёра». Для неузнанных названий — фоллбэк по группе/типу.
 */
@DrawableRes
...
/**
 * Вариант для экранов, где под рукой нет полной [ExerciseEntity] (история, итоги, редактор
 * программы). [type]/[group] используются только для фоллбэка неузнанных имён.
 */
@DrawableRes
fun iconResFor(
    name: String,
  ...
  when {
    ...
    n.contains("штанг") ||
        n.contains("гриф") ||
        n.contains("становая тяга") ||
        n.contains("румынская тяга") ||
    ...
    n.contains("тренаж") ||
        ...
        n.contains("хаммер") ||
        n.contains("гакк") ||
        n.contains("жим ногами") ||
        n.contains("разгибание ног") ||
        n.contains("сгибание ног") -> return R.drawable.ic_ex_machine
  }

  // Своё тело и корпус.
  when {
    n.contains("планк") ||
        n.contains("скручиван") ||
        n.contains("подъём ног") ||
        n.contains("подъем ног") ||
        n.contains("велосипед") ||
        n.contains("складка") ||
        n.contains("русские") -> return R.drawable.ic_ex_core
    n.contains("подтягиван") ||
        n.contains("отжиман") ||
        n.contains("брусья") ||
        n.contains("гиперэкстензия") ||
        n.contains("бёрпи") ||
        n.contains("берпи") ||
        n.contains("выпад") ||
        n.contains("турецкий") -> return R.drawable.ic_ex_bodyweight
  }
