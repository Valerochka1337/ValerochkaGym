...
/**
 * Two independent segmental analyses from InBody. Percentages are the values printed by the device
 * relative to its reference, not an application-generated health verdict.
 */
data class InBodySegmentValues(
  ...
  val hasAnyValue: Boolean
    get() =
        leanMassKg != null || leanPercentage != null || fatMassKg != null || fatPercentage != null
}

fun BodyMeasurementEntity.inBodySegmentValues(segment: InBodySegment): InBodySegmentValues =
    when (segment) {
      InBodySegment.LEFT_ARM ->
          InBodySegmentValues(
