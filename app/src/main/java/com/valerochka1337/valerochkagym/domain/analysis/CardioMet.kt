...
/**
 * Метаболический эквивалент (МЕТ) кардио-подхода — во что превращается «30 минут на дорожке
 * со скоростью 9 км/ч», чтобы сравнить его с рекомендацией ВОЗ.
 *
 * ВОЗ рекомендует 150–300 минут умеренной или 75–150 минут интенсивной аэробной активности
 * в неделю, что даёт примерно **500–1000 МЕТ-минут в неделю** — это и есть целевая полоса.
 * Границы интенсивности: 3.0–5.9 МЕТ — умеренная, ≥ 6.0 — интенсивная.
 *
...
object CardioMet {
    ...
    /**
     * МЕТ подхода по названию упражнения и параметрам. [speedKmh] и [inclinePct] используются
     * только там, где они осмысленны (дорожка); для прочих тренажёров — табличное значение.
     */
    fun forSet(exerciseName: String, speedKmh: Double?, inclinePct: Double?): Double {
        val name = exerciseName.lowercase()
        return when {
            name.contains("дорожк") || name.contains("беговая") ->
                treadmillMet(speedKmh, inclinePct)
    ...
    /**
     * Уравнения ACSM: ходьба до 6.4 км/ч, бег выше. В разрыве 6.4–8 км/ч берётся беговое
     * уравнение — на этой скорости человек уже переходит на бег.
     *
