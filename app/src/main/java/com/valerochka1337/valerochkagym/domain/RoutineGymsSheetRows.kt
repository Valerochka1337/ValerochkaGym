...
object RoutineGymsSheetRowMapper {
    ...
    fun rows(record: RoutineGymsSheetRecord): List<List<Any?>> = when (record) {
        is RoutineGymsSheetRecord.Snapshot -> snapshotRows(record)
        is RoutineGymsSheetRecord.Tombstone -> listOf(deletion(record.routineSyncId, record.updatedAt))
    }
    ...
    private fun snapshotRows(snapshot: RoutineGymsSheetRecord.Snapshot): List<List<Any?>> {
        ...
        val gymIds = snapshot.gymSyncIds
            .map { requireCanonicalSheetUuid(it, "gym_id") }
            .toSortedSet()
    ...
    private data class SnapshotBuilder(
        ...
        fun toRecord(): RoutineGymsSheetRecord? {
            ...
            return when (isDeleted) {
                ...
                false -> RoutineGymsSheetRecord.Snapshot(
