...
/**
 * Обратный к [BodyMeasurementRowMapper]. Поддерживает как полный формат A:AP, так и ранний A:N:
 * отсутствующие после N показатели остаются null. UUID — ключ дедупликации, поэтому импортированные
 * записи сразу получают [UploadStatus.UPLOADED] и не отправляются обратно.
 */
object BodyMeasurementRowParser {
  ...
  private fun String.toDoubleLoose(): Double? =
      takeIf(String::isNotEmpty)?.replace(',', '.')?.toDoubleOrNull()?.takeIf(Double::isFinite)
