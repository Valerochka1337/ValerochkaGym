...
data class ExerciseStatistics(
    ...
    val hasData: Boolean get() = lastPerformedAt != null
}

/**
 * Статистика одного упражнения по выполненным подходам завершённых тренировок.
 *
 * У разных типов упражнения разная сравнимая величина: для силовых — оценочный максимум,
 * для упражнений на время — суммарная длительность, для кардио — расчётная дистанция.
 */
class ExerciseStatisticsCalculator @Inject constructor() {
    ...
    private fun strength(
        ...
        val points = sessions.mapNotNull { sets ->
            val best = sets.mapNotNull { row ->
                OneRepMax.epley(row.weightKg, row.reps)?.let { row to it }
        ...
        val bestEstimate = validSets.mapNotNull { row ->
            OneRepMax.epley(row.weightKg, row.reps)?.let { row to it }
        }.maxByOrNull { it.second }
        val heaviest = validSets.maxByOrNull { it.weightKg ?: 0.0 }
        val bestVolume = sessions.maxByOrNull { sets ->
    ...
    private fun timed(
        ...
        return ExerciseStatistics(
            ...
            points = points,
            lastPerformedAt = last?.firstOrNull()?.completedAt,
            lastSummary = last.orEmpty().mapNotNull { it.durationSec?.takeIf { value -> value > 0 }?.let(::duration) }
                .joinToString(", "),
            records = buildList {
                if (longestSet > 0) add(ExerciseStatisticRecord("Самый долгий подход", duration(longestSet)))
                if (longestSession > 0) add(ExerciseStatisticRecord("Максимум за тренировку", duration(longestSession)))
            },
        )
    }
        ...
        return ExerciseStatistics(
            ...
            records = buildList {
                if (longestDistance > 0.0) {
                    add(ExerciseStatisticRecord("Самая длинная дистанция", "${number(longestDistance)} км"))
                }
                if (maxSpeed > 0.0) add(ExerciseStatisticRecord("Максимальная скорость", "${number(maxSpeed)} км/ч"))
                if (maxIncline > 0.0) add(ExerciseStatisticRecord("Максимальный наклон", "${number(maxIncline)}%"))
            },
        )
    }
    ...
    private fun number(value: Number?): String {
        ...
        return BigDecimal.valueOf(decimal).setScale(1, java.math.RoundingMode.HALF_UP)
