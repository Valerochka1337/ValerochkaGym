...
/**
 * Управление активной (незавершённой) тренировкой. В любой момент времени может существовать
 * не более одной активной тренировки — методы [startFromRoutine]/[startEmpty] защищены от
 * создания второй и возвращают id уже существующей.
 */
interface ActiveWorkoutRepository {

    /**
     * Создаёт тренировку по программе [routineId]: имя = имя программы, упражнения по позициям,
     * подходы из plannedSets с предзаполнением значениями «прошлого раза» (по индексу подхода).
     * Если активная тренировка уже есть — возвращает её id без создания новой.
     */
    ...
    /**
     * Добавляет упражнение в тренировку: позиция = max + 1, с одним подходом, предзаполненным
     * первым подходом «прошлого раза» (если есть). Возвращает id нового workout_exercise.
     */
