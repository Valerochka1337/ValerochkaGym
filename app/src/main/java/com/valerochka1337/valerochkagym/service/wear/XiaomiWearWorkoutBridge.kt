...
constructor(
  ...
  /**
   * Передаёт только свежее live-измерение. Когда монитор сам сбрасывает просроченное значение, RPK
   * получает один пустой пакет и перестаёт рисовать его как живой.
   */
  ...
  private fun handleWatchMessage(bytes: ByteArray) {
    val message =
        runCatching { json.decodeFromString<WatchMessage>(bytes.decodeToString()) }.getOrNull()
            ?: return
    ...
    when (message.type) {
      ...
      TYPE_PING -> sendPong()
      TYPE_COMMAND ->
          message.toCommandOrNull()?.let { command ->
  ...
  private fun rememberCommand(id: String?): Boolean =
      synchronized(lock) {
  ...
  private fun WatchMessage.toCommandOrNull(): WatchCommand? =
      when (command) {
        COMMAND_ADD_REST_SECONDS ->
            seconds
