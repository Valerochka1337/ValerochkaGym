 ...
 */
@Singleton
class XiaomiWearTransport @Inject constructor(
    ...
    private val messageListener = object : OnMessageReceivedListener {
        override fun onMessageReceived(nodeId: String, message: ByteArray) {
    ...
    override fun send(message: ByteArray) {
        ...
        mainHandler.post {
            ...
            runCatching {
                messageApi.sendMessage(destination, copy)
    ...
    private fun discoverNode() {
        ...
        runCatching {
            nodeApi.getConnectedNodes()
                ...
                .addOnFailureListener(
                    OnFailureListener {
                        mainHandler.post { discoveryInFlight = false }
                    },
                )
        }.onFailure {
            discoveryInFlight = false
    ...
    private fun ensureDeviceManagerPermission(currentNodeId: String) {
        ...
        runCatching {
            authApi.checkPermission(currentNodeId, Permission.DEVICE_MANAGER)
                .addOnSuccessListener(
    ...
    private fun requestDeviceManagerPermission(currentNodeId: String) {
        ...
        permissionRequestedNodeId = currentNodeId
        runCatching {
            authApi.requestPermission(currentNodeId, Permission.DEVICE_MANAGER)
                .addOnSuccessListener(
                    OnSuccessListener {
                        mainHandler.post {
                ...
                .addOnFailureListener(
                    OnFailureListener {
                        mainHandler.post { permissionRequestInFlight = false }
                    },
                )
        }.onFailure {
            permissionRequestInFlight = false
    ...
    private fun registerMessageListener(currentNodeId: String) {
        ...
        runCatching {
            messageApi.addListener(currentNodeId, messageListener)
                .addOnSuccessListener(
                    OnSuccessListener {
                        mainHandler.post {
                ...
                .addOnFailureListener(
                    OnFailureListener {
                        mainHandler.post { listenerRegistrationInFlight = false }
                    },
                )
        }.onFailure {
            listenerRegistrationInFlight = false
