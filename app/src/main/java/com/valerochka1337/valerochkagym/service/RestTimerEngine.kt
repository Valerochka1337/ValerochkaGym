...
sealed interface RestTimerState {
   ...
   *
   * [endsAtMillis] — момент окончания по стенным часам ([System.currentTimeMillis]). Это источник
   * истины: [remainingSec] всегда вычисляется из него, а не накапливается тиками.
   */
  data class Timed(
      val totalSec: Int,
   ...
   * измерение игнорируется: [startedAtMillis] задаёт нижнюю границу времени пакета, который может
   * начать отсчёт. [belowSinceMillis] сбрасывается при высоком или пропавшем пульсе.
   */
...
constructor(
  ...
  /**
   * Принимает уже проверенное на свежесть измерение. Пакет, полученный до старта отдыха, намеренно
   * не используется. Значение выше порога начинает выдержку заново.
   */
