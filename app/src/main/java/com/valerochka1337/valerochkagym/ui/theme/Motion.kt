 ...
 * ([MaterialTheme.motionScheme]); сторонние анимационные библиотеки не подключаем (см.
 ...
 * * статические спеки навигации ([NavSlideSpec], [NavFadeSpec], [TabFadeSpec]) — enter/exit лямбды
 *   `NavHost` не @Composable, до `MaterialTheme` оттуда не дотянуться, поэтому спеки заданы
 *   константами с теми же характеристиками, что пружины Expressive.
 *
 * spatial — для движения и размеров (слайды, масштаб, высота), effects — для «бесформенных» свойств
...
object GymMotion {
  ...
  /**
   * Сдвиг экрана при push/pop и выезде модалок. Сдвиг и затухание живут в ОДНОМ тайминге, иначе
   * контент успевает стать непрозрачным, пока экран ещё едет, — читается как «тормозит».
  ...
  /**
   * Кроссфейд между вкладками нижнего меню — быстрый, предсказуемый, без слайда. Единственный tween
   * в приложении: сиблингам направление не положено (§7 дизайн-системы).
   */
