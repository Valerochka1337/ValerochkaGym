        ...
        viewModelScope.launch {
          ...
          try {
            ...
            if (release == null) {
              ...
              val shouldPrompt =
                  release.tagName != ignoredTag && release.tagName != dismissedPromptTag
              _uiState.update {
                it.copy(
    ...
    val release =
        when (val status = _uiState.value.status) {
          ...
          is AppUpdateStatus.Failed ->
              status.release.takeIf { status.retry == AppUpdateRetry.DOWNLOAD }
          else -> null
            ...
            val file =
                repository.downloadAndVerify(release) { downloaded, total ->
                  _uiState.update { current ->
                    if (
                        (current.status as? AppUpdateStatus.Downloading)?.release?.tagName !=
                            release.tagName
                    ) {
                      current
                    } else {
