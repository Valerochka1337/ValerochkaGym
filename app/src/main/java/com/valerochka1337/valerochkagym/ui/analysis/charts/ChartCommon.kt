...
/**
 * Общие правила отрисовки графиков (одни и те же во всех карточках вкладки «Анализы»): тонкие
 * метки, сплошная волосяная сетка на тон от поверхности, подписи — текстовыми токенами, а не цветом
 * данных.
 */
object ChartSpec {
...
@Composable
fun chartLabelStyle(): TextStyle =
    MaterialTheme.typography.labelSmall.copy(
        color = MaterialTheme.colorScheme.onSurfaceVariant,
    )

/**
 * Округлённая шкала значений: границы и шаг подобраны так, чтобы подписи получались «круглыми» (0 /
 * 50 / 100), а не 0 / 47.3 / 94.6.
 *
 * Ноль включается по требованию ([Companion.forRange] с `zeroBased`). Для столбцов он обязателен —
 * длина столбца иначе врёт о величине. Для линии тренда, наоборот, вреден: рост силы на 20% от
 * собственного веса штанги на шкале «от нуля» превращается в плоскую линию. Поэтому тренд рисуется
 * в своём диапазоне, а честность обеспечивают подписи оси.
 */
@Immutable
data class NiceScale(
  ...
  companion object {
    /**
     * Шкала, покрывающая [rawMin]..[rawMax] примерно с [targetTicks] делениями. При [zeroBased]
     * нижняя граница притягивается к нулю. Пустые/нулевые данные дают шкалу 0..1, чтобы график не
     * схлопнулся.
     */
    fun forRange(
        rawMin: Float,
