...
/**
 * Семантическая обёртка тактильного отклика: экраны зовут «что произошло» ([confirm], [step]…), а
 * не низкоуровневый [HapticFeedbackType]. Единая карта «событие → паттерн» держит отклик
 * согласованным по всему приложению, а настройка `hapticsEnabled` выключает его в одном месте (все
 * методы становятся no-op).
 *
 * Экраны получают экземпляр через [LocalGymHaptics]; корень приложения кладёт его в композицию из
 * настроек (см. MainActivity).
 */
class GymHaptics
...
@Composable
fun gymHaptics(): GymHaptics = LocalGymHaptics.current ?: rememberGymHaptics(enabled = false)
