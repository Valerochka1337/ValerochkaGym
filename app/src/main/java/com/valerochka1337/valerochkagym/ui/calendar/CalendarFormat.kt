...
fun dayTitle(date: LocalDate): String =
    date.format(DateTimeFormatter.ofPattern("d MMMM", RU))
...
/**
 * 42 ячейки сетки месяца [yearMonth], начиная с понедельника недели, содержащей 1-е число, и заканчивая
 * заполнением до 6 недель. Ячейки вне [yearMonth] помечены `inMonth = false`.
 *
 * Точка ячейки: [DotStyle.Completed] (залитая), если в этот день была завершённая тренировка
 * ([completedDays]); иначе [DotStyle.Planned] (контурная) для сегодня/будущего, если день запланирован —
 * есть ad-hoc ([adHocDays]) ИЛИ его день недели входит в недельное расписание ([weeklyIsoDays]); иначе
 * [DotStyle.None]. Точки считаются только для дней внутри месяца.
 */
fun buildMonthCells(
    yearMonth: YearMonth,
    today: LocalDate,
    ...
    return (0 until CALENDAR_CELLS).map { offset ->
        ...
        DayCellUi(
            ...
            inMonth = inMonth,
            isToday = date == today,
            dot = if (!inMonth) DotStyle.None else dotFor(date, today, completedDays, adHocDays, weeklyIsoDays),
        )
    }
}
...
private fun dotFor(
    ...
    !date.isBefore(today) && (date in adHocDays || date.dayOfWeek.value in weeklyIsoDays) -> DotStyle.Planned
    else -> DotStyle.None
}
