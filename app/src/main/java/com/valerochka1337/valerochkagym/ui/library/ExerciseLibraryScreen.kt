...
/**
 * The exercise library: search field, muscle-group filter chips and a list of exercises. A FAB
 * opens the editor sheet for creating a custom exercise.
 *
 * When [onExerciseSelected] is non-null the screen acts as a picker: tapping a row invokes the
 * callback. В режиме просмотра тап по строке открывает разметку мышц упражнения — тот же редактор,
 * что и при создании (см. [ExerciseEditorSheet]).
 */
@Composable
    ...
    Box(modifier = Modifier.fillMaxSize()) {
      Column(
        ...
        val horizontalPadding =
            if (windowWidthClass == GymWindowWidthClass.Compact) 16.dp else 24.dp
        ...
        when {
          ...
          exercises.isEmpty() ->
              ...
              FadeInContent {
                LazyColumn(
                    modifier = Modifier.fillMaxSize().testTag(EXERCISE_CATALOG_LIST_TAG),
                  ...
                  items(exercises, key = { it.id }) { exercise ->
                    ExerciseRow(
                        ...
                        modifier =
                            Modifier.testTag("exercise_catalog_row_${exercise.id}")
...
private fun LibraryHeader(onBack: () -> Unit) {
  Row(
      modifier =
          Modifier.fillMaxWidth().padding(start = 8.dp, end = 24.dp, top = 12.dp, bottom = 8.dp),
        ...
        Row {
          ...
          IconButton(onClick = onOpenFilters) {
            Icon(
                ...
                tint =
                    if (filtersActive) MaterialTheme.colorScheme.primary
      ...
      SheetChipRow("Каталог") {
        ExerciseCatalogOrigin.entries.forEach { origin ->
          GymFilterChip(
              ...
              { onOrigin(origin) },
              origin.originLabel(),
              count = counts.origins[origin],
          )
        }
      }
      SheetChipRow("Группа мышц") {
        GymFilterChip(state.filters.group == null, onClearGroup, "Все", count = counts.groups[null])
...
private fun ExerciseCatalogTypeFilter.typeLabel() =
    when (this) {
