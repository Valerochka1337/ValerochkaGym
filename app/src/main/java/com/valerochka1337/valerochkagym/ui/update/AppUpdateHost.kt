...
fun AppUpdateHost(
    ...
    AlertDialog(
        ...
        text = {
          Column {
            ...
            Text(
                text =
                    "Размер обновления — ${formatUpdateBytes(release.apk.sizeBytes)}. " +
        ...
        dismissButton = {
          Row {
            ...
            TextButton(onClick = onDismissPromptOnce) { Text("Позже") }
          }
        },
    )
  }
    ...
    AlertDialog(
        ...
        text = {
          Column(modifier = Modifier.fillMaxWidth()) {
            ...
            if (total > 0L) {
              val progress = (downloading.downloadedBytes.toFloat() / total).coerceIn(0f, 1f)
        ...
        confirmButton = { TextButton(onClick = onHideDownloadDialog) { Text("Скрыть") } },
    )
  }
  ...
  state.errorDialogMessage?.let { message ->
    AlertDialog(
        ...
        dismissButton = { TextButton(onClick = onDismissError) { Text("Позже") } },
    )
  }
  ...
  pendingUnknownSourcesIntent?.let { intent ->
    AlertDialog(
        ...
        text = {
          Text(
              "Система блокирует установку файлов из новых источников. Разрешите " +
                  "Yarumo coach устанавливать приложения — затем установка продолжится " +
                  "автоматически.",
          )
        },
        confirmButton = {
        ...
        dismissButton = {
          TextButton(onClick = { pendingUnknownSourcesIntent = null }) { Text("Позже") }
        },
    )
  }
}
