 ...
 * превращает график в кашу. Полные значения по всем точкам читаются в таблице под графиком, поэтому
 * выбор точки ничего не «запирает».
 *
 * Базовая линия не нулевая (см. [NiceScale]), поэтому заливки под линией нет: она читалась бы как
 * «объём от нуля» и завышала бы разницу.
 *
 * Линия тренда — обычная линейная регрессия по всем точкам; рисуется приглушённым цветом, чтобы не
 * спорить с данными, и только при ≥ 3 точках (по двум точкам тренда не бывает).
 */
@Composable
fun TrendLineChart(
...
/**
 * Пересчёт данных в пиксели: поля под подписи, шкала значений и позиции точек. Считается один раз
 * на отрисовку и повторно — на тапе, чтобы попадание считалось по той же геометрии.
 */
private class LineGeometry(
  ...
  fun x(index: Int): Float {
    ...
    val t =
        if (span <= 0f) index.toFloat() / (points.size - 1)
        else (points[index].xMillis - minX) / span
    return plot.left + t * plot.width
  }

  fun y(value: Float): Float = plot.bottom - scale.fraction(value) * plot.height

  fun nearestIndex(tapX: Float): Int = points.indices.minBy { abs(x(it) - tapX) }
}
...
private fun DrawScope.drawTrend(
  ...
  drawLine(
      ...
      start =
          Offset(
              geometry.plot.left,
              geometry.y(startValue.coerceIn(geometry.scale.min, geometry.scale.max)),
          ),
      end =
          Offset(
              geometry.plot.right,
              geometry.y(endValue.coerceIn(geometry.scale.min, geometry.scale.max)),
          ),
...
private fun DrawScope.drawMarkers(
  ...
  points.forEachIndexed { index, point ->
    ...
    val radius =
        if (isEmphasized) ChartSpec.MarkerRadius.toPx() else ChartSpec.MarkerRadius.toPx() * 0.6f
