        ...
        Column(
            ...
            day.adHoc.forEach { item ->
                ...
                GymCard(
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        if (item.canStart) {
                            PillButton(text = "Начать", onClick = { onStartAdHoc(item) }, modifier = Modifier.weight(1f))
                        }
                        TextButton(onClick = { onCancelAdHoc(item.scheduledId) }) {
                            Text("Удалить")
                        }
                    }
                }
            ...
            day.recurring?.let { rule ->
                ...
                GymCard(
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        if (rule.canStart) {
                            PillButton(text = "Начать", onClick = { onStartRecurring(rule.routineId) }, modifier = Modifier.weight(1f))
                        }
