 ...
 * приглушённым, чтобы неполные данные не читались как провал.
 */
@Immutable
...
/**
 * Столбчатый график по периодам (неделям). Один ряд данных — значит один цвет для всех
 * столбцов и никакой легенды: подпись карточки уже говорит, что отложено.
 *
 * Опорная линия [referenceValue] (например средняя за период) — сплошная волосяная линия
 * поверх столбцов: она отвечает на вопрос «эта неделя выше или ниже обычного», ради которого
 * иначе пришлось бы вводить вторую ось.
 */
@Composable
fun ColumnChart(
    ...
    Canvas(
        ...
        drawSelectedValue(geometry, data, colors, labelStyle, textMeasurer, valueFormatter, selectedIndex)
    }
}
...
private class ColumnGeometry(
    ...
    val slotWidth: Float get() = plot.width / data.size

    /**
     * Толщина столбца: не толще [ChartSpec.MaxBarWidth] и не больше 60% слота — столбец никогда
     * не заполняет свою полосу целиком, воздух между столбцами и есть разделитель.
     */
    val barWidth: Float = minOf(
...
private fun DrawScope.drawColumnGrid(
    ...
    geometry.scale.ticks.forEach { tick ->
        ...
        drawText(
            ...
            topLeft = Offset(geometry.plot.left - layout.size.width - 6.dp.toPx(), y - layout.size.height / 2f),
        )
    }
}
