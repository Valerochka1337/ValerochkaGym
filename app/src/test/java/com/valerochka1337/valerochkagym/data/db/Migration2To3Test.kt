 ...
 *
 * Главная проверка — не «таблица появилась», а то, что после цепочки миграций базу открывает **сам
 * Room**: он сверяет фактическую схему с текущей ожидаемой версией и падает на любом расхождении в
 * типах, ключах или индексах. Это ловит расхождение [GymDatabase.MIGRATION_2_3] с сгенерированной
 * схемой, которое отдельными `SELECT`-ами не увидеть.
 */
