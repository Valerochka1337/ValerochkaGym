...
/**
 * Opt-in local recovery gate. It only reads [VALEROCHKA_GYM_DB_COPY], then migrates an internal
...
class EmulatorV11CopyMigrationTest {
    ...
    @After fun tearDown() { context.deleteDatabase(targetName) }
    ...
    fun `external emulator copy migrates through production registry without changing source`() {
        ...
        assumeTrue("VALEROCHKA_GYM_DB_COPY is required for this local-only gate", source?.isFile == true)
        ...
        try {
            ...
            assertEquals("migration must not modify the source database", sourceShaBefore, sha256(sourceFile))
        } finally {
    ...
    private fun readV11Provenance(source: File): Set<String> {
        SQLiteDatabase.openDatabase(source.path, null, SQLiteDatabase.OPEN_READONLY).use { database ->
            ...
            return database.rawQuery("PRAGMA foreign_key_check", null).use { cursor ->
                buildSet {
                    while (cursor.moveToNext()) add("${cursor.getString(0)}|${cursor.getLong(1)}|${cursor.getString(2)}|${cursor.getInt(3)}")
                }
            }
        }
    }
    ...
    private fun assertRecoveredSchema(
        ...
        database.query("SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('exercise_variants', 'exercise_variant_muscles')").use { cursor ->
            assertTrue(!cursor.moveToFirst())
        }
        database.query("PRAGMA foreign_key_check").use { cursor ->
            ...
            assertEquals("migration must not add foreign-key violations", sourceForeignKeyViolations, afterMigration)
        }
    }
}
