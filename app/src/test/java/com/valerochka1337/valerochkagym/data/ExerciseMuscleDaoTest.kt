...
class ExerciseMuscleDaoTest : RoomDaoTest() {
  ...
  fun `getForExercise returns muscles from the strongest involvement down`() = runTest {
    ...
    assertEquals(listOf(Muscle.UPPER_CHEST, Muscle.TRICEPS, Muscle.FRONT_DELTS), muscles)
  }

  @Test
  fun `upsert overwrites the contribution of an already stored muscle`() = runTest {
    val exerciseId = insertExercise("Жим штанги лёжа", MuscleGroup.CHEST)
    muscleDao.upsertAll(listOf(ExerciseMuscleEntity(exerciseId, Muscle.UPPER_CHEST, 80)))

    muscleDao.upsertAll(listOf(ExerciseMuscleEntity(exerciseId, Muscle.UPPER_CHEST, 100)))
  ...
  @Test
  fun `registry reconcile preserves legacy id links and leaves custom maps authoritative`() =
      runTest {
        val legacy =
            ExerciseEntity(
  ...
  fun `reconcile leaves custom exercise with built in display name untouched`() = runTest {
    ...
    assertEquals(
        listOf(Muscle.LOWER_BACK to 0),
        muscleDao.getForExercise(id).map { it.muscle to it.contribution },
    )
  ...
  fun `replaceForExercise drops muscles that are no longer selected`() = runTest {
    ...
    muscleDao.replaceForExercise(
        exerciseId,
        listOf(ExerciseMuscleEntity(exerciseId, Muscle.LATS, 90)),
    )
  ...
  fun `seeding uses the curated map for a catalogue exercise`() = runTest {
    ...
    val expected =
        requireNotNull(
                CanonicalExerciseRegistry.loadsFor(
                    ExerciseEntity(
                        id = exerciseId,
                        name = "Жим штанги лёжа",
                        muscleGroup = MuscleGroup.CHEST,
  ...
  fun `catalogue uses canonical role loads across exercises`() {
    val squatQuads =
        seedExerciseMuscles
            .getValue("приседания со штангой")
            .single { it.muscle == Muscle.QUADS }
            .contribution
    val treadmillQuads =
        seedExerciseMuscles
            .getValue("беговая дорожка")
            .single { it.muscle == Muscle.QUADS }
            .contribution
