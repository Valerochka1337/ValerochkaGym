...
class Migration25To26Test {
  ...
  @After
  fun cleanup() {
    ApplicationProvider.getApplicationContext<Context>().deleteDatabase("relations-25-26.db")
    ApplicationProvider.getApplicationContext<Context>().deleteDatabase("relations-1-26.db")
  }

  @Test
  fun `relations migration retains exact journals and isolates participant operations`() {
    helper.createDatabase("relations-25-26.db", 25).use {
      it.execSQL(
          "INSERT INTO health_sync_outbox(operationId,scope,requestBytes,requestSha256,dispatched) VALUES('op','owner',X'200a7b7d','hash',1)"
      )
    }
    helper
        .runMigrationsAndValidate("relations-25-26.db", 26, true, GymDatabase.MIGRATION_25_26)
        .use {
  ...
  @Test
  fun `full historical migration retains routine and creates empty relations journal`() {
    helper.createDatabase("relations-1-26.db", 1).use {
      it.execSQL("INSERT INTO routines(id,name,note) VALUES(1,'Ноги','Примечание')")
    }
    helper
        .runMigrationsAndValidate("relations-1-26.db", 26, true, *GymDatabase.ALL_MIGRATIONS)
        .use {
          it.query("SELECT note FROM routines WHERE id=1").use { c ->
            assertTrue(c.moveToFirst())
            assertEquals("Примечание", c.getString(0))
          }
          it.query("SELECT COUNT(*) FROM coach_relation_operations").use { c ->
            c.moveToFirst()
