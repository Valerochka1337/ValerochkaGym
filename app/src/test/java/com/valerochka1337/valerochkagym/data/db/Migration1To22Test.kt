...
class Migration1To22Test {
  ...
  @Test
  fun `full migration creates the notes schema`() {
    helper.createDatabase(name, 1).use {
      it.execSQL("INSERT INTO routines(id,name,note) VALUES(1,'Ноги','')")
    }
    helper.runMigrationsAndValidate(name, 22, true, *GymDatabase.ALL_MIGRATIONS).use { db ->
      db.query("SELECT COUNT(*) FROM exercise_personal_hints").use {
        it.moveToFirst()
