...
class Migration1To24Test {
  ...
  @Test
  fun `full migration keeps historical workout data and creates an empty health ledger`() {
    helper.createDatabase(name, 1).use {
      it.execSQL("INSERT INTO routines(id,name,note) VALUES(1,'Ноги','')")
    }
    helper.runMigrationsAndValidate(name, 24, true, *GymDatabase.ALL_MIGRATIONS).use { db ->
      db.query("SELECT COUNT(*) FROM routines WHERE id=1").use {
        it.moveToFirst()
        assertEquals(1, it.getInt(0))
      }
      db.query("SELECT COUNT(*) FROM health_logical_records").use {
        it.moveToFirst()
