 ...
 * заголовком `Authorization` в каждом методе (а не через OkHttp-интерсептор), потому что получение
 * access-токена — suspend-операция ([GoogleAuth.getAccessToken]), которую нельзя выполнить внутри
 * синхронного интерсептора.
 *
 * Base URL — `https://sheets.googleapis.com/`. Неизвестные поля ответов игнорируются (см. настройку
 * `Json` в DI), поэтому DTO описывают только нужные части.
 */
interface SheetsApi {
...
data class SheetPropertiesDto(
    ...
    @EncodeDefault(EncodeDefault.Mode.NEVER) val index: Int? = null,
    @EncodeDefault(EncodeDefault.Mode.NEVER) @SerialName("sheetId") val sheetId: Int? = null,
)
...
data class BatchRequestDto(
    @EncodeDefault(EncodeDefault.Mode.NEVER) val addSheet: AddSheetDto? = null,
    @EncodeDefault(EncodeDefault.Mode.NEVER) val insertDimension: InsertDimensionDto? = null,
)
...
/**
 * Тело `values.append`. [values] собирается вручную ([JsonArray] из [JsonArray]) из row mapper-ов
 * тренировок и замеров, чтобы числа уходили числами, а `null` — пустой строкой (см.
 * `SheetsRepositoryImpl.cellToJson`).
 */
@Serializable
