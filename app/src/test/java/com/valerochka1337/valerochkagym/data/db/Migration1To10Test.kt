...
class Migration1To10Test {
    ...
    fun `room opens v9 variant fixture preserving every base row and set`() {
        createV1().use { db ->
            ...
            db.execSQL("INSERT INTO routines VALUES (1, '22222222-2222-2222-2222-222222222222', 2, 'A', '')")
            db.execSQL("INSERT INTO exercise_variants VALUES (1, '33333333-3333-3333-3333-333333333333', 1, 'Узкий хват', 'узкий хват', 0, 3)")
            db.execSQL("INSERT INTO routine_exercises VALUES (2, 1, 1, '33333333-3333-3333-3333-333333333333', 4, 90, '[]')")
            db.execSQL("UPDATE workout_exercises SET variantSyncId = '33333333-3333-3333-3333-333333333333', variantNameSnapshot = 'Узкий хват' WHERE id = 1")
        ...
        sql.query("SELECT id, workoutExerciseId, setIndex, weightKg, reps, durationSec, speedKmh, inclinePct, isCompleted, completedAt FROM workout_sets ORDER BY id").use { cursor ->
            assertTrue(cursor.moveToFirst())
            assertEquals(10L, cursor.getLong(0)); assertEquals(1L, cursor.getLong(1)); assertEquals(0, cursor.getInt(2))
            assertEquals(70.0, cursor.getDouble(3), 0.0); assertEquals(10, cursor.getInt(4)); assertEquals(30, cursor.getInt(5))
            assertEquals(9.5, cursor.getDouble(6), 0.0); assertEquals(4.0, cursor.getDouble(7), 0.0); assertEquals(1, cursor.getInt(8)); assertEquals(1000L, cursor.getLong(9))
            assertTrue(cursor.moveToNext())
            assertEquals(11L, cursor.getLong(0)); assertEquals(1L, cursor.getLong(1)); assertEquals(1, cursor.getInt(2))
            assertEquals(72.5, cursor.getDouble(3), 0.0); assertEquals(8, cursor.getInt(4)); assertEquals(45, cursor.getInt(5))
            assertEquals(8.0, cursor.getDouble(6), 0.0); assertEquals(2.0, cursor.getDouble(7), 0.0); assertEquals(0, cursor.getInt(8)); assertTrue(cursor.isNull(9))
        }
        sql.query("SELECT id, routineId, exerciseId, position, restSeconds, plannedSetsJson FROM routine_exercises WHERE id = 2").use { cursor ->
            assertTrue(cursor.moveToFirst()); assertEquals(2L, cursor.getLong(0)); assertEquals(1L, cursor.getLong(1)); assertEquals(1L, cursor.getLong(2))
            assertEquals(4, cursor.getInt(3)); assertEquals(90, cursor.getInt(4)); assertEquals("[]", cursor.getString(5))
        }
        sql.query("SELECT sectionId, position FROM workout_exercises WHERE id = 1").use { cursor ->
            assertTrue(cursor.moveToFirst()); assertTrue(cursor.getString(0).isNotBlank()); assertEquals(0, cursor.getInt(1))
        }
        sql.query("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'exercise_variants'").use { cursor ->
            assertTrue(!cursor.moveToFirst())
        }
        ...
        sql.query("PRAGMA table_info('workout_exercises')").use { cursor ->
            val columns = mutableSetOf<String>(); while (cursor.moveToNext()) columns += cursor.getString(cursor.getColumnIndexOrThrow("name"))
            assertTrue("variantSyncId" !in columns && "variantNameSnapshot" !in columns && "sectionId" in columns)
        }
        ...
        sql.query("PRAGMA index_list('routine_exercises')").use { cursor ->
            val indexes = mutableSetOf<String>(); while (cursor.moveToNext()) indexes += cursor.getString(cursor.getColumnIndexOrThrow("name"))
            assertTrue("index_routine_exercises_routineId" in indexes && "index_routine_exercises_exerciseId" in indexes)
        }
        sql.query("PRAGMA foreign_key_list('workout_exercises')").use { cursor ->
            val referenced = mutableSetOf<String>(); while (cursor.moveToNext()) referenced += cursor.getString(cursor.getColumnIndexOrThrow("table"))
    ...
    fun `migration removes variant columns while preserving routine sections and sets`() {
        val callback = object : SupportSQLiteOpenHelper.Callback(9) {
            override fun onCreate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL("CREATE TABLE routine_exercises (id INTEGER PRIMARY KEY NOT NULL, routineId INTEGER NOT NULL, exerciseId INTEGER NOT NULL, variantSyncId TEXT, position INTEGER NOT NULL, restSeconds INTEGER, plannedSetsJson TEXT NOT NULL)")
                db.execSQL("CREATE TABLE workout_exercises (id INTEGER PRIMARY KEY NOT NULL, workoutId TEXT NOT NULL, exerciseId INTEGER NOT NULL, sectionId TEXT NOT NULL, variantSyncId TEXT, variantNameSnapshot TEXT, position INTEGER NOT NULL)")
                db.execSQL("CREATE TABLE workout_sets (id INTEGER PRIMARY KEY NOT NULL, workoutExerciseId INTEGER NOT NULL, setIndex INTEGER NOT NULL, weightKg REAL, reps INTEGER, durationSec INTEGER, speedKmh REAL, inclinePct REAL, isCompleted INTEGER NOT NULL, completedAt INTEGER)")
            }
            override fun onUpgrade(db: SupportSQLiteDatabase, oldVersion: Int, newVersion: Int) = Unit
        }
        val sql = FrameworkSQLiteOpenHelperFactory().create(
            SupportSQLiteOpenHelper.Configuration.builder(context).name(name).callback(callback).build(),
        ).writableDatabase
        sql.execSQL("INSERT INTO routine_exercises VALUES (7, 3, 4, 'variant', 2, 60, '[]')")
        sql.execSQL("INSERT INTO workout_exercises VALUES (8, 'w', 4, 'section', 'variant', 'Узкий хват', 3)")
        sql.execSQL("INSERT INTO workout_sets VALUES (9, 8, 1, 80, 8, NULL, NULL, NULL, 1, 1000)")
        GymDatabase.MIGRATION_9_10.migrate(sql)
        sql.query("SELECT id, routineId, exerciseId, position, restSeconds FROM routine_exercises").use { cursor ->
            assertTrue(cursor.moveToFirst()); assertEquals(7L, cursor.getLong(0)); assertEquals(2, cursor.getInt(3)); assertEquals(60, cursor.getInt(4))
        }
        sql.query("SELECT sectionId, position FROM workout_exercises").use { cursor ->
            assertTrue(cursor.moveToFirst()); assertEquals("section", cursor.getString(0)); assertEquals(3, cursor.getInt(1))
        }
        sql.query("SELECT isCompleted, completedAt FROM workout_sets").use { cursor ->
            assertTrue(cursor.moveToFirst()); assertEquals(1, cursor.getInt(0)); assertEquals(1000L, cursor.getLong(1))
        }
        sql.query("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'exercise_variants'").use { cursor -> assertTrue(!cursor.moveToFirst()) }
        sql.close()
    }
}
