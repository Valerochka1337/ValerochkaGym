...
/** Opens a real v1 file through every handwritten migration and lets Room validate v10. */
...
class Migration1To10Test {
    ...
    @Test
    fun `room opens v1 data through v10 with legacy section and durable health schema intact`() {
        createV1().use { db ->
        ...
        db.openHelper.writableDatabase.query("SELECT COUNT(*) FROM measurement_snapshots").use { cursor ->
            assertTrue(cursor.moveToFirst())
            assertEquals(0, cursor.getInt(0))
        }
        db.openHelper.writableDatabase.query("SELECT COUNT(*) FROM health_sync_outbox").use { cursor ->
            assertTrue(cursor.moveToFirst())
            assertEquals(0, cursor.getInt(0))
        }
        db.close()
