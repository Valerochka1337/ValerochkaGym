...
class RecurrenceTest {
    ...
    fun `nextOccurrence returns from itself when the weekday already matches`() {
        ...
        val millis = nextOccurrenceMillis(isoDay = 1, hour = 18, minute = 30, zone = ZoneOffset.UTC, from = from)
    ...
    fun `nextOccurrence advances to the next matching weekday`() {
        ...
        val from = LocalDate.of(2026, 8, 3)
        val millis = nextOccurrenceMillis(isoDay = 3, hour = 9, minute = 0, zone = ZoneOffset.UTC, from = from)
    ...
    fun `nextOccurrence wraps to next week for an earlier weekday`() {
        ...
        val millis = nextOccurrenceMillis(isoDay = 1, hour = 7, minute = 15, zone = ZoneOffset.UTC, from = from)
