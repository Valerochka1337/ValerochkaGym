...
class GymDaoTest : RoomDaoTest() {
    ...
    @Test
    fun `routine and workout relations expose gyms and diagnostics only report active workouts`() = runTest {
        val alpha = gymDao.insertGym(GymEntity(name = "Альфа"))
        val beta = gymDao.insertGym(GymEntity(name = "Бета"))
        ...
        assertEquals(setOf(alpha, beta), routineDao.getRoutineWithExercises(routineId)!!.gyms.map { it.id }.toSet())
        assertEquals(setOf(alpha, beta), gymDao.getGymsForWorkout("active").map { it.id }.toSet())
        assertEquals(setOf(alpha, beta), workoutDao.getWorkoutFull("active")!!.gyms.map { it.id }.toSet())
