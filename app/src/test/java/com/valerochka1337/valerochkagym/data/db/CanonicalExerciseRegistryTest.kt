...
class CanonicalExerciseRegistryTest {
    @Test fun `catalogue has unique explicit rows and complete legacy bridges`() {
    ...
    @Test fun `explicit movement patterns have canonical role maps`() {
        val entries = CanonicalExerciseRegistry.entries
        assertTrue(entries.all { it.loads == CanonicalExerciseRegistry.canonicalLoads(it.movementPattern) })
        fun named(name: String) = entries.first { it.exercise.name == name }
        fun primary(entry: CanonicalExerciseRegistry.Entry, muscle: Muscle) = entry.loads.any { it.muscle == muscle && it.contribution == 100 }
        ...
        assertEquals(CanonicalExerciseRegistry.MovementPattern.LATERAL_RAISE, named("Подъём руки в сторону в кроссовере").movementPattern)
        assertTrue(primary(named("Подъём руки в сторону в кроссовере"), Muscle.SIDE_DELTS))
        assertEquals(CanonicalExerciseRegistry.MovementPattern.REAR_DELT_FLY, named("Обратный пек-дек").movementPattern)
        assertTrue(primary(named("Обратный пек-дек"), Muscle.REAR_DELTS))
        listOf("Наружная ротация блока", "Внутренняя ротация блока").forEach { assertTrue(primary(named(it), Muscle.ROTATOR_CUFF)) }
        ...
        assertEquals(CanonicalExerciseRegistry.MovementPattern.HORIZONTAL_PULL, named("Тяга одной рукой в кроссовере").movementPattern)
        assertTrue(primary(named("Тяга одной рукой в кроссовере"), Muscle.LATS))
        assertEquals(CanonicalExerciseRegistry.MovementPattern.HIP_ABDUCTION, named("Отведение бедра в кроссовере").movementPattern)
        assertTrue(primary(named("Отведение бедра в кроссовере"), Muscle.HIP_ABDUCTORS))
        assertEquals(CanonicalExerciseRegistry.MovementPattern.HIP_ADDUCTION, named("Приведение бедра в кроссовере").movementPattern)
        assertTrue(primary(named("Приведение бедра в кроссовере"), Muscle.ADDUCTORS))
        assertEquals(CanonicalExerciseRegistry.MovementPattern.TIBIALIS_RAISE, named("Подъём носков").movementPattern)
        assertTrue(primary(named("Подъём носков"), Muscle.TIBIALIS_ANTERIOR))
        listOf("Подъём руки в сторону в кроссовере", "Обратный пек-дек", "Тяга одной рукой в кроссовере", "Отведение бедра в кроссовере", "Приведение бедра в кроссовере").forEach { name ->
            assertTrue(named(name).loads.none { it.muscle in setOf(Muscle.UPPER_CHEST, Muscle.LOWER_CHEST) && it.contribution == 100 })
        }
        assertTrue(named("Беговая дорожка").loads.all { it.contribution == 50 })
    }

    @Test fun `catalogue covers approved families and excludes impossible pairs`() {
        val coverage = CanonicalExerciseRegistry.entries.flatMap { it.coverage }.toSet()
        assertTrue(setOf("chest","shoulders","back","arms","legs","core","powerlifting","calisthenics","cardio").all(coverage::contains))
        assertTrue(setOf("barbell","dumbbell","machine","cable","bodyweight","rings","kettlebell","cardio_machine").all(coverage::contains))
    ...
    @Test fun `every static row parses and legacy core rows keep conservative core loads`() {
        val entries = CanonicalExerciseRegistry.entries
