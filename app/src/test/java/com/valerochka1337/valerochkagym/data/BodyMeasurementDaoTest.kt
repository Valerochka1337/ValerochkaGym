...
class BodyMeasurementDaoTest : RoomDaoTest() {
  ...
  fun `measurements persist in descending date order and keep nullable values`() = runTest {
    dao.insert(
        BodyMeasurementEntity(id = "old", measuredAt = 1_000, weightKg = 70.0, waistCm = null)
    )
    dao.insert(
        BodyMeasurementEntity(id = "new", measuredAt = 2_000, weightKg = null, chestCm = 96.0)
    )
  ...
  fun `not uploaded query returns pending and failed measurements only`() = runTest {
    ...
    dao.insert(
        BodyMeasurementEntity(id = "failed", measuredAt = 2_000, uploadStatus = UploadStatus.FAILED)
    )
    dao.insert(
        BodyMeasurementEntity(id = "done", measuredAt = 1_000, uploadStatus = UploadStatus.UPLOADED)
    )
