...
class WorkoutImportRepositoryTest : RoomDaoTest() {
  ...
  fun `rejects conflicting v9 section tuple before writing local workout`() = runTest {
    val base =
        dataRow("w-v9", "2026-01-02", "10:00", "T", "Жим", "Грудь", "Силовое", "1", "80", "8") +
            listOf(
    ...
    val api =
        FakeSheetsApi(
            ...
            values = mutableListOf(v9Header, base, conflict),
        )
  ...
  fun `imports an InBody measurement and marks it uploaded without requeueing`() = runTest {
    ...
    val api =
        FakeSheetsApi(
            ...
            valuesByRange =
                mapOf(
                    "Measurements!A:AP" to
                        listOf(
                            BodyMeasurementRowMapper.HEADER_ROW,
                            BodyMeasurementRowMapper.row(measurement).map {
                              it?.toString().orEmpty()
                            },
                        ),
                ),
            ...
            valuesByRange =
                mapOf(
                    "Routines!A:M" to
                        listOf(
                            RoutineRowMapper.HEADER_ROW,
                            RoutineRowMapper.deletion("routine-1", 200).map {
                              it?.toString().orEmpty()
                            },
                        ),
                ),
  ...
  fun `database error during insert surfaces as failure`() = runTest {
    ...
    val failingExercises =
        object : ExerciseDao by db.exerciseDao() {
          override suspend fun insert(exercise: ExerciseEntity): Long =
              throw RuntimeException("db insert failed")
        }
    val api =
        FakeSheetsApi(
            sheets = mutableListOf("Workouts"),
  ...
  @Test
  fun `exercise import keeps registry built ins local while applying valid custom legacy rows`() =
      runTest {
        ...
        assertEquals(
            ...
            db.exerciseMuscleDao().getForExercise(custom.id).associate {
              it.muscle to it.contribution
            },
        )
      }
  ...
  private fun settingsRepository(spreadsheetId: String?): SettingsRepository {
    val prefs =
        if (spreadsheetId == null) emptyPreferences()
  ...
  private class FakeSheetsApi(
    ...
    override suspend fun batchUpdate(
        bearer: String,
        spreadsheetId: String,
        body: com.valerochka1337.valerochkagym.data.google.BatchUpdateRequestDto,
    ...
    override suspend fun appendValues(
        ...
        spreadsheetId: String,
        range: String,
        body: com.valerochka1337.valerochkagym.data.google.AppendValuesDto,
    ...
    override suspend fun updateValues(
        ...
        spreadsheetId: String,
        range: String,
        body: com.valerochka1337.valerochkagym.data.google.UpdateValuesDto,
        valueInputOption: String,
    ): JsonElement = JsonNull
  }
