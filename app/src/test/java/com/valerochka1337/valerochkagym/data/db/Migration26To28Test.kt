...
class Migration26To28Test {
  ...
  fun `clean v26 adds durable coach contract`() {
    val cleanName = "clean-live-coach-26-28.db"
    try {
      ...
      helper.runMigrationsAndValidate(cleanName, 28, true, *GymDatabase.ALL_MIGRATIONS).use { db ->
        db.query(
  ...
  fun `duplicate populated set identities fail without changing the original database`() {
    ...
    assertThrows(android.database.sqlite.SQLiteException::class.java) {
      helper.runMigrationsAndValidate(name, 28, true, *GymDatabase.ALL_MIGRATIONS).close()
    }
  ...
  fun `migration assigns immutable set uuid and preserves legacy null semantics`() {
    ...
    helper.runMigrationsAndValidate(name, 28, true, *GymDatabase.ALL_MIGRATIONS).use { db ->
      ...
      GymDatabase.MIGRATION_27_28.migrate(db)
