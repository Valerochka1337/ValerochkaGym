...
class WorkoutRowParserTest {
  ...
  fun `v9 rows retain section identity and ignore variant values`() {
    ...
    val row =
        WorkoutRowMapper.rows(workout).single().map { it?.toString().orEmpty() }.toMutableList()
  ...
  fun `v9 rows reject a conflicting base tuple within one section`() {
    ...
    val rows =
        WorkoutRowMapper.rows(workout).map { row ->
          row.map { it?.toString().orEmpty() }.toMutableList()
        }
    rows[1][15] = "1"
  ...
  fun `accepts comma decimal separator`() {
    ...
    assertEquals(
        100.5,
        WorkoutRowParser.parse(rows).workouts.single().exercises.single().sets.single().weightKg!!,
        0.0,
    )
  }
  ...
  private fun exercise(
      ...
      WorkoutExerciseWithSets(
          workoutExercise =
              WorkoutExerciseEntity(id = 0, workoutId = "w-1", exerciseId = 0, position = position),
