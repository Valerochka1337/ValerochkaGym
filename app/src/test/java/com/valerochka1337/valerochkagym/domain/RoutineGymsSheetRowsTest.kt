...
class RoutineGymsSheetRowsTest {
    ...
    fun `routine gyms header and rows contain a sorted full gym set`() {
        val snapshot = RoutineGymsSheetRecord.Snapshot(
    ...
    fun `routine gyms parser selects the snapshot with greatest updatedAt`() {
        val old = RoutineGymsSheetRecord.Snapshot(
            routineSyncId = ROUTINE_ID,
            updatedAt = 100,
            gymSyncIds = setOf(GYM_ID_A),
        )
        val current = RoutineGymsSheetRecord.Snapshot(
            routineSyncId = ROUTINE_ID,
            updatedAt = 200,
            gymSyncIds = setOf(GYM_ID_A, GYM_ID_B),
        )
        val rows = listOf(RoutineGymsSheetRowMapper.HEADER_ROW) +
    ...
    fun `empty routine gym selection remains live and differs from tombstone`() {
        val empty = RoutineGymsSheetRecord.Snapshot(
            routineSyncId = ROUTINE_ID,
            updatedAt = 200,
            gymSyncIds = emptySet(),
        )
        ...
        val parsedDeletion = RoutineGymsSheetRowParser.parse(
            listOf(RoutineGymsSheetRowMapper.deletion(ROUTINE_ID, 300).map { it?.toString().orEmpty() }),
        )
