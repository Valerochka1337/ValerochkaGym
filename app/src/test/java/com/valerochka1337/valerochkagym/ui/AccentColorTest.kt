 ...
 * цветов темы и `activity-alias` с иконкой в манифесте. Разъехавшийся `aliasName` не сломает
 * сборку — он просто оставит пользователя со старой иконкой, поэтому связка проверяется тестом.
 */
...
class AccentColorTest {
    ...
    fun `fromId returns the saved accent`() {
        AccentColor.entries.forEach { accent ->
            assertEquals(accent, AccentColor.fromId(accent.id))
        }
    }
