 ...
 * друга), тап мимо фигуры — `null`, а раскладка мышц по видам покрывает всю модель, кроме тех, что
 * осознанно оставлены спискам.
 */
...
class BodyMapHitTest {
  ...
  fun `every persisted muscle belongs to a visible sector`() {
    ...
    assertTrue(
        Muscle.LOWER_CHEST in muscleSectors(BodyView.FRONT).first { it.slug == "chest" }.members
    )
    assertTrue(
        Muscle.UPPER_BACK in muscleSectors(BodyView.BACK).first { it.slug == "upper-back" }.members
    )
  }
  ...
  fun `an interior point of a muscle picks that same muscle`() {
    ...
    BodyView.entries.forEach { view ->
      ...
      parsed.sectors.forEach { shape ->
        ...
        if (hit != shape.sector.defaultMuscle)
            collisions += "$view ${shape.sector.slug} @($x,$y) → $hit"
    ...
    assertTrue(
        "области наехали друг на друга: ${collisions.joinToString("; ")}",
        collisions.isEmpty(),
    )
  }
