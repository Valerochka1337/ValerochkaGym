 ...
 *
 * Отдельно проверяется случай «все значения одинаковые» — одна тренировка в периоде или
 * повторённый вес. Раньше он давал диапазон шириной в тысячную долю: шаг оказывался мельче
 * точности Float, `value += step` не сдвигал значение, и построение меток не заканчивалось
 * никогда — график ронял приложение по памяти.
 */
class NiceScaleTest {
    ...
    fun `a flat trend sits in the middle of its scale`() {
        ...
        assertTrue("линия не должна лежать на краю: ${scale.fraction(58.33f)}", scale.fraction(58.33f) in 0.2f..0.8f)
    }
