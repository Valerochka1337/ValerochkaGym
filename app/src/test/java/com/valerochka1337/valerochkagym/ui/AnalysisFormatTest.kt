...
class AnalysisFormatTest {
    ...
    @Test
    fun `every balance has a title and both side labels`() {
        BalanceId.entries.forEach { balance ->
            val (left, right) = balance.sideLabels()
            assert(balance.title().isNotBlank())
            assert(left.isNotBlank() && right.isNotBlank())
        }
    }
}
