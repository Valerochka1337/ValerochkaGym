...
| T-003 | pass | Single implementation writer | T-001,T-002 | Delayed-projection Compose regression passes and verifies index zero. |
...
| AC-005 | T-003 | `*ExerciseLibraryScreenTest`: scroll deep, apply query, assert no eager reset, await projected results, assert index zero | pass |
...
| `./gradlew :app:testDebugUnitTest --tests '*ExerciseLibraryScreenTest'` | pass | Follow-up Compose regression: `BUILD SUCCESSFUL in 6s` (37 actionable tasks: 3 executed, 34 up-to-date). |
| `./gradlew :app:testDebugUnitTest` | pass | Final stable diff after the scroll follow-up: `BUILD SUCCESSFUL in 15s` (37 actionable tasks: 1 executed, 36 up-to-date). |
| `./gradlew :app:assembleDebug` | pass | Final stable diff after the scroll follow-up: `BUILD SUCCESSFUL in 1s` (42 actionable tasks: 3 executed, 39 up-to-date). |
...
- Resolved: scroll reset keys on projected `state.query`; the Compose regression proves immediate input does not reset early and the matching result resets to index zero.
