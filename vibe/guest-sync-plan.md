...
| AC-002 | A cache owned by A is never treated as guest data for B; the safe A→B purge still retains the standard catalog. |
...
2. **Claim protocol.** A fresh database is `GUEST`; claim is a transaction which first rejects active work and any `CLAIMED(other)` state, then records B and `mergeId` without clearing guest tables. `OWNED(A)→B` is the only branch that purges personal data and A baseline/outbox. `CLAIMED(B)` is retained across sign-out and token loss; it is not reclassified as guest.
