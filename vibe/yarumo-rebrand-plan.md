...
and pulse for themed monochrome. Legacy density PNGs are regenerated from the approved icon mark
for non-adaptive launchers. `app_name` and listed hardcoded display strings become `Yarumo coach`.
...
- Source assets are only `docs/branding/yarumo-coach/app-icon-source.png` (launcher mark) and
  ...
  center it, and retain the existing 0.7 adaptive safe zone. Do not rasterize the wordmark into an
...
- Keep `ic_launcher*` resource names, four `activity-alias` names/order, manifest component
  ...
  Aliases may retain distinct existing background/palette resources; every foreground uses the same
  approved mark. Round references an adaptive icon; monochrome is a readable `VectorDrawable` mask,
...
| T-006 | `vibe/yarumo-rebrand-plan-track.md` | root session | T-005 | Stable diff: run full unit tests then debug assembly once; inspect debug APK metadata (`applicationId`, version, label) using installed Android build tools. If configured release signing inputs are available, run `./gradlew :app:assembleRelease`; otherwise record the exact signing blocker. If an emulator/device is available, install successive compatible debug APKs without clearing data and verify package/data/auth remain present via ADB/accessibility tree. | Final commands/results and any unavailable environment blocker recorded. | AC-001–AC-005 |
...
- Verify XML/resource resolution under all aliases, day/night and font-scale host configuration;
  ...
  metadata; release R8 when signing is available; and upgrade without clear data when a device is
...
- Risks: adaptive crop/themed mask legibility (safe inset + XML/vector/resource tests); resource
  ...
  or asset parser (explicit invariant audit). Rollback restores resources/copy only; no data changes.
