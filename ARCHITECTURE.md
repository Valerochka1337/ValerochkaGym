...
- **UploadConfigurationWorker** — снимки упражнений и залов либо tombstone зала. Уникальные
  ...
  транзакцией сохраняет каталог, мышцы, требования и (для активной сессии) строку тренировки
...
карту мышц и требования оборудования; `Gyms` — версионированное оснащение зала;
`RoutineGyms` — набор залов версии программы. Старые столбцы сохраняют позиции: `Exercises`
расширен с A:J до A:L состоянием требований и ID оборудования, `Gyms` — с A:E до A:G
признаком настроенного оснащения и ID оборудования.
...
конфликт откатывает транзакцию целиком. Legacy-снимки залов восстанавливают прежние ограничения
только для ещё не настроенных залов;
они не откатывают новое оснащение. Формат различает неизвестные требования и явно пустой набор,
повторное восстановление не создаёт дубли связей. Отсутствие новых листов означает старую таблицу
без ограничений по залам.
