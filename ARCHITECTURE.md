...
Read-only доступ явно предоставляется только разрешённому системному обработчику и только на URI
этого файла: это сохраняет узкую модель доступа и обходит потерю Intent grant между внутренними
компонентами установщика на некоторых прошивках ColorOS/OxygenOS.
