...
Room v15, схемы коммитятся в `app/schemas/`, миграции только рукописные
(`MIGRATION_1_2` … `MIGRATION_14_15`) — `fallbackToDestructiveMigration` запрещён. Замеры тела лежат отдельно в `body_measurements`: все
...
и неэкспортируемым ключом Android Keystore. Refresh сериализован Mutex. Google-вход
через Credential Manager передаёт ID token и одноразовый серверный nonce; backend проверяет
подпись и audience. Email-вход включает регистрацию, подтверждение и восстановление пароля.
...
локальной/серверной версии. База принадлежит одному аккаунту и не отправляется другому.
