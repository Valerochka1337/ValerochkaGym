...
- **ИИ-черновик упражнения.** Библиотека берёт снимок всех упражнений и их карт мышц, отправляет
  его вместе с описанием в OpenRouter и принимает только строгий JSON. Ответ валидируется локально:
  существующий ID обязан быть в снимке, а новая карта использует известные мышцы, уникальные веса
  5–100 с шагом 5 и хотя бы одну целевую мышцу 100%. Room меняется только после подтверждения в
  существующем редакторе упражнения.
...
Во внешний free endpoint уходят только описание и каталог упражнений с картами мышц, поэтому UI и
README предупреждают не передавать конфиденциальные или лишние персональные данные.
