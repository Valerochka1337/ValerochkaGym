...
Room v5, схемы коммитятся в `app/schemas/`, миграции только рукописные
(`MIGRATION_1_2`, `MIGRATION_2_3`, `MIGRATION_3_4`, `MIGRATION_4_5`) —
`fallbackToDestructiveMigration` запрещён. Замеры тела лежат отдельно в `body_measurements`: все
...
повторных append-запросов. Нынешний app-managed `Measurements` занимает A:AP; при первом
экспорте старого заголовка A:N репозиторий безопасно вставляет 28 столбцов после N и дописывает
только новые заголовки, не переписывая исторические строки.

## OpenRouter-интеграция

`OpenRouterExerciseAiGenerator` вызывает OpenAI-совместимый `chat/completions` через отдельный
Retrofit-инстанс. Используется фиксированный free endpoint
`google/gemma-4-26b-a4b-it:free`, а не меняющийся `openrouter/free`: так одна модель
обрабатывает один и тот же строгий контракт. `response_format=json_schema` вместе с
`provider.require_parameters=true`, подробными описаниями полей схемы и низкой температурой (`0.1`)
снижают вероятность неразбираемого ответа. Запрос не стримится: шторка ждёт атомарный JSON и либо
открывает существующее упражнение, либо передаёт новый черновик в `ExerciseEditorSheet`; локальная
валидация проверяет используемую ветку `kind`, известный id и карту мышц, но терпимо игнорирует
неиспользуемые поля другой ветки. Ошибки сети, лимита и разбора остаются в шторке для повтора.

Тот же фиксированный endpoint читает фото InBody через multimodal message (инструкция + JPEG data
URL) и отдельную строгую schema. В запрос попадает только выбранный снимок, а извлекаются только
